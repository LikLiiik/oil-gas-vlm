...
def test_wl_high_resistivity_dense_zone_is_not_called_water():
    model = downstream.get("well_log_analyzer")
    depth = np.linspace(1600.0, 1610.0, 101, dtype=np.float32)
    curves = {
        "depth": depth,
        "RT": np.full(101, 600.0, dtype=np.float32),
        "DEN": np.full(101, 2.66, dtype=np.float32),
        "CNL": np.full(101, 0.09, dtype=np.float32),
    }
    out = model.detect(
        {"analysis_type": "fluid_identification"},
        context={
            "curves": curves,
            "formation_tops": [{"formation": "MNKT", "depth_m": 1603.4}],
        },
    )
    assert out
    assert out[0]["class_name"] == "high_resistivity_anomaly"
    assert out[0]["fluid_type"] != "water"
    assert out[0]["formation_tops"][0]["formation"] == "MNKT"
    assert "candidate_not_confirmed_fluid_type" in out[0]["interpretation_limitations"]


def test_facies_results_are_explicitly_unlabeled_attribute_clusters():
    model = downstream.get("facies_classifier")
    y, x = np.mgrid[:30, :40]
    array = (np.sin(x / 4.0) + np.cos(y / 5.0)).astype(np.float32)
    out = model.detect(
        {"n_clusters": 3, "method": "gmm"},
        context={"array": array},
    )
    assert out
    assert all(item["class_name"].startswith("attribute_cluster_") for item in out)
    assert all(item["interpretation_status"] == "unlabeled_attribute_cluster" for item in out)
