...
def _skip_if_no_demo():
    if not DEMO_RUN.is_dir():
        raise SystemExit(f"demo run dir 不存在: {DEMO_RUN}")
...
def test_aggregate_skips_detections_out_of_roi():
    ...
    per_class = aggregate_adapter_detections(dets, m, (10, 20, 50))
    # 只有 in_roi=False 的这条被跳过；channel（不同类别）不受影响
    assert "channel" in per_class
    if "fault plane" in per_class:
        # inline 检测被跳过，crossline 的 fault 检测仍然写入
        assert per_class["fault plane"][3, :, :].max() == 0
