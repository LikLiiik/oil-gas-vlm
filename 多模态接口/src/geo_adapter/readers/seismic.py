    ...
    try:
        with segyio.open(str(path), "r", strict=False, ignore_geometry=False) as handle:
            ...
            if "inline" in config.processing.seismic.views:
                views["inline"] = _view("inline", np.asarray(handle.iline[ilines[ii]]), "inline", ("crossline", "sample"), {"inline": int(ilines[ii])}, config)
            if "crossline" in config.processing.seismic.views:
                views["crossline"] = _view("crossline", np.asarray(handle.xline[xlines[xi]]), "crossline", ("inline", "sample"), {"crossline": int(xlines[xi])}, config)
            ...
            metadata = {"inline_count": len(ilines), "crossline_count": len(xlines), "sample_count": samples, "lazy_slice_policy": True}
