...
DEFAULT_MIN_PIXELS = int(os.environ.get(
    "OIL_GAS_VLM_MIN_PIXELS", str(64 * 28 * 28),
))
DEFAULT_MAX_PIXELS = int(os.environ.get(
    "OIL_GAS_VLM_MAX_PIXELS", str(384 * 28 * 28),
))
...
class VLMClient:
    """Qwen2.5-VL/Qwen3-VL client with lazy, inference-only loading."""
    ...
    def load(self):
        ...
        print(f"[VLM] loading {self.model_path} ({model_type}) ...")
        ...
        if model_type in {"qwen2_5_vl", "qwen3_vl", "qwen3_vl_moe"}:
            ...
            print(
                f"[VLM] image pixel range: "
                f"{self.min_pixels}..{self.max_pixels} per image"
            )
