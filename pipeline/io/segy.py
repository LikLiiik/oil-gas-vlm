...
def write_attribute_segy_like(
    ref_segy: str,
    attribute: np.ndarray,
    out_path: str,
) -> str:
    """Copy a reference SEG-Y and replace traces without loading its cube."""
    import shutil
    import segyio

    reference = Path(ref_segy)
    if not reference.is_file():
        raise FileNotFoundError(f"reference SEG-Y not found: {reference}")
    attribute = np.asarray(attribute, dtype=np.float32)
    if attribute.ndim != 3:
        raise ValueError(f"attribute must be 3D, got shape {attribute.shape}")

    with segyio.open(str(reference), "r", strict=False, ignore_geometry=False) as src:
        expected = (len(src.ilines), len(src.xlines), len(src.samples))
    if attribute.shape != expected:
        raise ValueError(
            f"attribute shape {attribute.shape} != reference geometry {expected}"
        )

    shutil.copyfile(reference, out_path)
    with segyio.open(str(out_path), "r+", strict=False, ignore_geometry=False) as dst:
        for il_index, inline_number in enumerate(dst.ilines):
            dst.iline[int(inline_number)] = attribute[il_index]
        dst.flush()
    return out_path
