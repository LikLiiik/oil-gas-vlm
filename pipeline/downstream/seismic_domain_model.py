...
def gradient_fault_prob(arr: np.ndarray, sigma: float = 3.0) -> np.ndarray:
    ...
    # arr has canonical layout (sample, trace): reflectors primarily create
    # sample-axis gradients, whereas faults break lateral trace continuity.
    sample_gradient = sobel(arr, axis=0, mode="reflect")
    lateral_gradient = sobel(arr, axis=1, mode="reflect")
    faultiness = np.abs(lateral_gradient) - 0.5 * np.abs(sample_gradient)
...
class SeismicDomainDetector:
    ...
    def detect(self, instruction: dict, image=None,
        ...
        if rois:
            ...
            # ROI is additional sensitive guidance, not a hard crop. A rough
            # VLM box must not suppress fault evidence elsewhere in the slice.
            all_results.extend(self._detect_on_array(
            ...
            for rj in regions[:ri]:
                ...
                if x_overlap > 0 and y_overlap > 0:
                    ...
                    if iou > 0.3:
                        ...
                        break
            if not suppressed:
                component_prob = np.zeros(arr.shape, dtype=np.float32)
                component_prob[r["ys"], r["xs"]] = prob[r["ys"], r["xs"]]
                out.append({
                    ...
                    "coordinate_shape": list(canvas_shape or arr.shape),
                    "_mask_offset": [offset_x, offset_y],
                    "_probability_map": component_prob,
                })
        return out
