"""Well Log ML Classifier — 基于公开数据集训练的测井岩性分类器。

使用 FORCE 2020 + 公开岩石物理数据训练 XGBoost/RandomForest。
首次使用时自动生成训练数据、训练模型并缓存权重到磁盘。
后续调用直接加载缓存模型。
...
class WellLogML:
    ...
    def _load(self):
        ...
        try:
            ...
            print("[well_log_ml] sklearn not available")
            return False

        print("[well_log_ml] training on synthetic petrophysical data ...")
        ...
        try:
            ...
            print(f"[well_log_ml] model saved to {CACHE_PATH}")
        except Exception:
            pass
        return True
