...
class CigFaultDetector:
    ...
    description = (
        ...
        "3D断层概率体预测。仅接受真实3D体/瓦片，自动下载权重(~40MB)"
    )
    required_fields = [
    ...
    def _volume_to_detections(prob_vol: np.ndarray, thr: float,
        ...
        for i in range(n_slices):
            ...
            for rid in range(1, n_regions + 1):
                ...
                component_prob = np.zeros(slc.shape, dtype=np.float32)
                component_prob[ys, xs] = slc[ys, xs]
                results.append({
                    ...
                    "coordinate_shape": list(slc.shape),
...
class CigChannelDetector:
    ...
    description = (
        ...
        "3D河道/沉积体分割。仅接受真实3D体/瓦片，自动下载权重(~40MB)"
    )
    required_fields = [
