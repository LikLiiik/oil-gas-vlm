...
class FaciesClassifier:
    ...
    def detect(self, instruction: dict, image=None,
        ...
        for cid in range(n_clusters):
            ...
            cluster_mask = np.zeros((ny, nx), dtype=bool)
            cluster_mask[c_ys, c_xs] = True

            results.append({
                "id": f"facies_cluster_{cid}",
                "class_name": f"attribute_cluster_{cid}",
                "cluster_id": int(cid),
                ...
                "interpretation_status": "unlabeled_attribute_cluster",
                "limitations": [
                    "not_a_named_geological_facies",
                    "requires_geometry_well_and_horizon_evidence_for_labeling",
                ],
                "model": self.name,
                "coordinate_space": "array",
                "coordinate_shape": [ny, nx],
                "_mask_array": cluster_mask,
            })
        return results
